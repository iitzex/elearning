            ...
            if course_cell:
                ...
                if link_tag:
                    ...
                    if course_link and course_link.startswith('/'):
                        course_link = "https://ap2.elearning.taipei" + course_link
...
def login_and_get_session(username, password):
    ...
    try:
        ...
        # 檢查是否登入成功
        # 正常的登入成功通常會跳轉回首頁，且頁面中會出現 "登出" 或您的姓名
        is_success = False
        if "logout" in response.text.lower() or "登出" in response.text or "個人選單" in response.text:
            is_success = True

        # 如果 post 之後沒發現關鍵字，檢查是否剛好在首頁但沒顯示關鍵字 (有些頁面可能比較複雜)
        if not is_success:
            # 嘗試抓一下首頁看看
            test_resp = session.get("https://elearning.taipei/mpage/")
    ...
    try:
        ...
        for link in links:
            ...
            if scorm_pattern in href:
                scorm_link = "https://ap2.elearning.taipei" + \
...
if __name__ == '__main__':
    ...
    with open("debug_courserecord.html", "w", encoding="utf-8") as f:
        f.write(response.text)

    courses = extract_course_info_from_html(response.text)

    if not courses:
        print("[警告] 找不到任何課程。請檢查 debug_courserecord.html 以確認頁面內容是否正確。")
        # 如果是讀取舊 cookie 導致的失敗，刪除它
        if logged_in and os.path.exists("cookies.json"):
    ...
    for course in courses:
        status = course.get('completion_status', '')
        if '未完成' in status or '進行中' in status:
    ...
    if incomplete_courses:
        ...
        for i, course in enumerate(incomplete_courses, 1):
            ...
            is_completed, progress, study_times, scorm_link = check_course_completion(
                session, course['link'])
